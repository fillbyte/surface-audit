...
such as `@v1.0.4` when you want fully reproducible workflow inputs.
  ...
  surface-audit:
    ...
    steps:
      ...
      - name: Install surface-audit
        run: python -m pip install surface-audit==1.0.4
...
Use `:latest` for convenience or a versioned tag such as `:1.0.4` when
