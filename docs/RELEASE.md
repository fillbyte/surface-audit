...
For example, after `v1.0.4` was released, both `v1.0.4` and `v1` point
...
uses: fillbyte/surface-audit@v1.0.4
```
...
latest compatible release on the `1.x` line." After `v1.0.4`, it is
normal for `v1` to point at the same commit as `v1.0.4`.
